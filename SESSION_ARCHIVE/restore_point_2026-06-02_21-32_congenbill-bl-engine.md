...
This session was a forms-scoping + first-B/L-build arc. Rebooting mid-slice. The
B/L code is UNCOMMITTED (survives reboot on disk; see Git/resume mechanics below).
Nothing was committed this session.

### Decisions locked this session
1. **Forms scope cut: 11 keep / 15 drop** (of the 26-doc kanban). Dropped = the
   third-party-ISSUED docs an agency receives but does not generate (all 4 customs,
   all 4 surveys, freight/demurrage/despatch, letter of protest, letter of indemnity,
   vessel particulars, pre-arrival checklist). Worksheet (with the 11-form build
   order + tally): `~/Documents/Claude/10_Projects/ShipOps/agency_docs/FORMS_TRIAGE.md`.
   Kanban reconciled: `…/agency_docs/kanban.html` — DEFAULT_DOCS trimmed to the 11 +
   a `DROPPED_NAMES` prune-on-load that drops the 15 from any already-seeded board.
   **The 11 keepers:** NOR, SOF, Cargo Manifest, Stowage Plan, Bill of Lading,
   Mate's Receipt, PDA, FDA, Agency Appointment Letter, Port Call Report, Agency Fee Invoice.
2. **Render path settled: pdfkit IS the engine; HTML templates are design-spec only.**
   Rejected headless-Chromium HTML→PDF (keeps services pure-Node, consistent with the
   earlier react-pdf rejection). The 4 existing HTML templates become visual reference,
   not code that runs.
3. **B/L scope: CONGENBILL first, engine-ready** — build one type now, structured so
   adding NORGRAIN/AMWELSH/etc. later = one registry entry, no renderer change.
4. **BIMCO copyright: reproduce clause text VERBATIM now**; licence (BIMCO SmartCon/
   IDEA for commercial generation) tracked as a SEPARATE commercial task, NOT a code
   blocker. Flagged in-file at top of bill-types.ts.

### Deprecated forms cache — /Users/wsd/dev/forms  (READ / COPY ONLY — never edit)
- `cpedit/config/bills_registry.json` — **19 B/L types** catalogued by trade + parent
  charter-party + copyright holder, and they partition cleanly by cargo physics
  (dry / liquid / gas = the carrier-classification rule). Many have sample PDFs at
  `cpedit/data/acquisition_candidates/<type>/candidate_00.pdf`.
- **CONGENBILL source used:** `cpedit/data/acquisition_candidates/congenbill/candidate_00.pdf`
  (official BIMCO CONGENBILL 2022, 2pp, text-based, quality score 100).
- Other dry-bulk samples on disk if/when we extend: austwheat, amwelsh, polcoal,
  fertivoy, cementvoy + cementvoybill, coal-orevoy, Norgrain-BL; gas: gasvoy_2005.
  (Liquid family: only shellvoy is parsed; tanker bills not yet acquired.)

### What got BUILT this session + VERIFIED (all uncommitted)
The CONGENBILL "engine": page-1 boxed data grid + page-2 verbatim Conditions of
Carriage. Renders a valid 2-page PDF. Files:
- **NEW** `packages/services/src/pdf/bills/bill-types.ts` — `BILL_TYPES` registry +
  `BillOfLadingType` + `BillTypeConfig` + `CONGENBILL_2022` (page-1 boilerplate +
  7 verbatim page-2 clauses). **Add a future B/L type HERE; renderer is unchanged.**
- **NEW** `packages/services/src/pdf/bills/render-bill.ts` — `renderBillOfLading(doc, input)`:
  absolute-positioned cell grid (page 1) + flowing clauses (page 2). Type-agnostic —
  reads whichever config `input.billType` selects.
- **MOD** `packages/services/src/pdf/port.ts` — `GenerateBillOfLadingInput` field model
  (each field annotated [platform] / [manual] / [b/l]) + `BillOfLadingParty` +
  `generateBillOfLading` added to `IPDFProvider`.
- **MOD** `packages/services/src/pdf/mock-adapter.ts` — `generateBillOfLading()` wired
  (reuses the existing `renderToBuffer` harness).
- **MOD** `packages/services/scripts/smoke-pdf.ts` — renders a realistic pet-coke
  CONGENBILL (Oxbow shipper / Sinochem notify / NOLA→Qingdao), asserts %PDF magic +
  exactly 2 pages.

Verification (green): `pnpm --filter @shipops/services exec tsc --noEmit` clean;
`pnpm --filter @shipops/services smoke-pdf` → FDA/SOF/B-L all valid %PDF, B/L = 2 pages.
pdftoppm render eyeballed — page-1 grid + page-2 clauses faithful to the BIMCO original.

**pdfkit gotcha logged for reuse:** it auto-spawns a blank page if a footer's
START_Y + full *wrapped* height exceeds the bottom margin (738pt on LETTER). The
~230-char BIMCO copyright wrapped to 3 lines and silently doubled the page count;
fixed with a 5.5pt footer at y=716. Rule: `start_y + lines×lineHeight ≤ 738`.

### ⛳ NEXT STEP on resume — ONE OPEN FORK (William dismissed the question; undecided)
The B/L is NOT user-reachable because Shipper / Consignee / Notify Party / B-L number /
number of originals / place of issue are **NOT in the port-call DB model**. So unlike
the FDA (one-click DB→PDF download), the B/L needs a **DATA-ENTRY FORM**. Decide the
UI shape before building the route:
  (a) **Full data-entry form** on the port-call page: platform pre-fills vessel /
      load-port / cargo from the port call; agent fills the cargo-trade chain → POST →
      PDF. The real product surface (bigger slice: form + route + Zod validation).
  (b) **Pre-filled editable draft** with blank party fields, downloadable immediately.
      Faster to ship, less complete.
  (c) **Decide the data model first** — capture shipper/consignee/notify upstream
      (a cargo-party model) vs. per-B/L entry. Shapes the form either way.
...
- Uncommitted (persists across reboot): `M packages/services/scripts/smoke-pdf.ts`,
  `M packages/services/src/pdf/mock-adapter.ts`, `M packages/services/src/pdf/port.ts`,
  `?? packages/services/src/pdf/bills/` (the 2 new files).
- Re-verify on return: `pnpm --filter @shipops/services exec tsc --noEmit` then
  `pnpm --filter @shipops/services smoke-pdf` (expect B/L = 2 pages).
- If you want a safety commit before more work: it's a clean, self-contained,
  verified unit (engine + renderer + smoke) — a reasonable `feat(services): CONGENBILL
  B/L pdfkit engine + smoke (no route/UI yet)` commit. NOT done — William hasn't asked.
